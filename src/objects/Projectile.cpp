...
void Projectile::Render(Application &app) {
  ...
  // We need to rotate it to point towards forward (+Z in local space of Matrix_Look_At)
  // Actually Matrix_Look_At already handles the orientation.
  // We just need to rotate the mesh 90 degrees around X so Y points to Z.
  glm::mat4 model =
      Matrix_Translate(m_Position.x, m_Position.y, m_Position.z) *
      Matrix_Look_At(m_Position, m_Position + forward, glm::vec4(0.0f, 1.0f, 0.0f, 0.0f)) *
