  ...
  for (const unsigned int &sgroup : sgroup_ids) {
    ...
    for (size_t shape = 0; shape < model->shapes.size(); ++shape) {
      ...
      for (size_t triangle = 0; triangle < num_triangles; ++triangle) {
        ...
        const glm::vec4 n =
            crossproduct(vertices[1] - vertices[0], vertices[2] - vertices[0]);
        for (size_t vertex = 0; vertex < 3; ++vertex) {
          tinyobj::index_t idx =
              model->shapes[shape].mesh.indices[3 * triangle + vertex];
  ...
  for (size_t shape = 0; shape < model->shapes.size(); ++shape) {
    ...
    for (size_t triangle = 0; triangle < num_triangles; ++triangle) {
      for (size_t vertex = 0; vertex < 3; ++vertex) {
        tinyobj::index_t idx =
            model->shapes[shape].mesh.indices[3 * triangle + vertex];
        out_data.indices.push_back(
            static_cast<unsigned int>(info.first_index + 3 * triangle + vertex)
        );
        ...
        if (idx.normal_index != -1) {
          out_data.normal_coefficients.push_back(
        ...
        if (idx.texcoord_index != -1) {
          out_data.texture_coefficients.push_back(
