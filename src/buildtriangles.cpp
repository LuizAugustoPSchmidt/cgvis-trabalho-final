...
GLuint BuildTriangles(GLsizei &num_indices, GLenum &type_indices) {
  ...
  //
  glBufferSubData(
      GL_ARRAY_BUFFER, 0, sizeof(NDC_coefficients), NDC_coefficients);
  ...
  // Aula_04_Modelagem_Geometrica_3D.pdf). Tal cor é definida como coeficientes
  // RGBA: Red, Green, Blue, Alpha; isto é: Vermelho, Verde, Azul, Alpha (valor
  // de transparência). Conversaremos sobre sistemas de cores nas aulas de
  ...
  glBufferData(
      GL_ARRAY_BUFFER, sizeof(color_coefficients), NULL, GL_STATIC_DRAW);
  glBufferSubData(
      GL_ARRAY_BUFFER, 0, sizeof(color_coefficients), color_coefficients);
  ...
  // GL_TRIANGLES na chamada glDrawElements() dentro de main(). Veja slides
  // 182-188 do documento Aula_04_Modelagem_Geometrica_3D.pdf.
  //
